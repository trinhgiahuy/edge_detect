...
#define MEM_BASE ((unsigned char *) 0x70000000)
  ...
  void check_register_access()
  {
    ...
    //
    SET_INPUT_OFFSET(0);
    if (0 != GET_INPUT_OFFSET) errors++;
    
    SET_INPUT_OFFSET(0xFF);
    if (0xFF != GET_INPUT_OFFSET) errors++;
    

    // output offset
    //
    SET_OUTPUT_OFFSET(0);
    if (0 != GET_OUTPUT_OFFSET) errors++;
    
    SET_OUTPUT_OFFSET(0xFF);
    if (0xFF != GET_OUTPUT_OFFSET) errors++;


    // height
    //
    SET_HEIGHT(0);
    if (0 != GET_HEIGHT) errors++;
    
    SET_HEIGHT(0xFF);
    if (0xFF != GET_HEIGHT) errors++;


    // width
    //
    SET_WIDTH(0);
    if (0 != GET_WIDTH) errors++;
    
    SET_WIDTH(0xFF);
    if (0xFF != GET_WIDTH) errors++;
    ...
    if (errors == 0) {
      ...
      printf("errors = %d \n", errors);
      #ifdef HOST
      SC_REPORT_ERROR("register test", "failed");
      #endif
    }
  }
