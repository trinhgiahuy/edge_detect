...
//
  ...
  // The offset for the start of image data is stored in bitmapoffset. The bmp_header1_read() and
  // bmp_header2_read() functions above read a total of 54 bytes of data from the bmp file. The bmp_palette_read()
  // function reads another (colorsused*4) bytes of data. Which leaves behind a number of bytes which we have to read out
  // before we get to the actual pixel data. The formula below calculates this number of bytes.
  extrabytestoread = int(bitmapoffset) - 54 - int(colorsused*4);

  // Read out the number of bytes calculated above from the input file.
  for (int i = 0; i < extrabytestoread; i++) { file_in.read ( &c, 1 ); }
...
//
//    05 March 2003
//    16 April 2020 - David Burnette - Fixed crash when using
//                    bmp_read_test() to read file written
//                    by bmp_08_write_test().
//
//  Author:
//
//    John Burkardt
